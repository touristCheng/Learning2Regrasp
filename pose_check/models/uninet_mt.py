...
class UniNet_MT(nn.Module):
	...
	def forward(self, sample):
		...
		if self.only_test:
			...
			loss_items = {'stable': [stable_pred, sample['stable'], 1.],
			              ...
			              'offset': [offset_pred, sample['offset'], 10.]
			              }
	...
	def calc_loss(self, loss_items: dict):
		...
		offset_loss = F.smooth_l1_loss(loss_items['offset'][1], loss_items['offset'][0], reduce=False)
		...
		return {'loss': total_loss, 'loss_items': [stable_loss, contact_loss, offset_loss]}
...
if __name__ == '__main__':
    model = UniNet_MT(mask_channel=True)
    points = torch.zeros((2, 4, 513))
    model(points)
